...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    render_path = os.path.join(model_path, name, "reduced_{}".format(iteration), "renders")
    gts_path = os.path.join(model_path, name, "reduced_{}".format(iteration), "gt")
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool):
    with torch.no_grad():
        ...
        _, indices = torch.topk(sum_scalings, k=1184790, largest=True)
